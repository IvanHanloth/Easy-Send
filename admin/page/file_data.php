...
<script>
    layui.use(['form', 'table','laypage','laydate'], function () {
        ...
        table.render({
            elem: '#main-table',
            url: '/admin/api/file_data_get.php',
            ...
            cols: [[
                ...
                {field: 'id', title: 'ID'},
                {field: 'origin',minWidth: 150, title: '文件名称'},
        ...
        table.on('toolbar(main-table-filter)', function (obj) {
            if (obj.event === 'delete') {  // 监听删除操作
                ...
                layer.confirm('真的删除这'+data.length+'行么', function (index) {
                    $.ajax({
                        type:"POST",
                        url:"/admin/api/file_data_delete.php",
        ...
        table.on('tool(main-table-filter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '../page/file_data_edit.php?id='+JSON.stringify(data.id),
                    end:function(){
                        table.reload('main-table',{})
                    }
                });
