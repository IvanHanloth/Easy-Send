...
if($_REQUEST["type"]=="get"){
    ...
    $result=mysqli_query($db,"SELECT `id`,`origin`,`times`,`tillday`,`uid` FROM `data` WHERE `type`='1' LIMIT {$start},{$limit}");
    ...
    $result=mysqli_query($db,"SELECT `id`,`origin`,`times`,`tillday`,`uid` FROM `data`".$add." LIMIT {$start},{$limit}");
}
