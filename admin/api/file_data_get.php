...
if($_REQUEST["type"]=="get"){
    ...
    $result=mysqli_query($db,"SELECT `id`,`origin`,`times`,`tillday` FROM `data` WHERE `type`='1' LIMIT {$start},{$limit}");
    ...
    $result=mysqli_query($db,"SELECT `id`,`origin`,`times`,`tillday` FROM `data`".$add." LIMIT {$start},{$limit}");
}
