...
function get_data()
{
    ...
    if ($_REQUEST["action"] == "get") {
        $count = $db->query( "SELECT count(*) FROM `data` WHERE `type`='2' ");
        $result = $db->query("SELECT `id`,`preview`,`times`,`tillday`,`uid` FROM `data` WHERE `type`='2' LIMIT {$start},{$limit}");
        ...
        $result = $db->query("SELECT `id`,`preview`,`times`,`tillday`,`uid` FROM `data`" . $add . " LIMIT {$start},{$limit}");
    }
...
function edit_data()
{
    ...
    if ($_REQUEST["type"] == "get") {
        ...
        return array("data" => $content, "tillday" => $data["tillday"], "times" => $data["times"]);
    }
