...
if($action=="upload"){
    ...
    echo return_json(array("token"=>$upToken,"rand"=>$rand,"bucket"=>$qiniu_bucket));
		...
		mysqli_query($db,"INSERT INTO `data` (`id`,`cloud_way` ,`gkey`, `type`, `data`,`origin`, `tillday`, `times` ,`uid`) VALUES (NULL, 'qiniu','{$key}', '1', '{$file_name}','{$origin_name}', '{$tilltime}', '{$times}','{$uid}')");
		//插入数据
		echo return_json(array("code"=>"200","tip"=>"文件上传成功","key"=>$key,"tillday"=>$tilltime,"times"=>$times,"qrcode"=>$qrcode.$domain."?key=".$key,"filename"=>$file_name));
		exit;
