...
$start = ($page - 1) * $limit;
$my_stmt = $db->prepare("SELECT count(*) FROM `data` WHERE `type`='1' AND `uid`=?");
...
$my_stmt = $db->prepare("SELECT `gkey`,`origin`,`times`,`tillday` FROM `data` WHERE `type`='1' AND `uid`=? LIMIT ?,?");
